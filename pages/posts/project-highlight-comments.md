...
const submitComment = async () => {
  ...
  // --- 【核心技术亮点：乐观更新 (Optimistic UI)】 ---
  // 【逻辑】不等网络请求，直接将数据插入前端数组头部（unshift），触发视图秒级重绘。
  // 【考点】极致的用户体验设计。掩盖网络延迟的标准做法。
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
        ...
        // 【逻辑】利用正则表达式将尖括号转义。
        // 【考点】"永远不要相信客户端传入的数据"。在入库前消毒，保证数据库层面的绝对纯净。
...
在技术探讨中如果涉及到这个组件的实现：“现在很多静态博客都有成熟的评论组件配置，你为什么要自己手写？”

**核心逻辑表达：**
“为了彻底打通这套全栈体验闭环。如果仅仅在配置文件里贴一行引入代码，那只能叫‘运维配置’，无法体现构建完整系统能力。
因为我本身通过 Serverless 已经连通了云端 Redis 数据库，所以我选择自己手写一个轻量级的 Vue3 表单组件和 API 接口。这样不仅深入实践了**组件的双向数据流转**、**网络请求封装**，我还特意在前端层面进行了**核心体验优化（如乐观更新）**和**安全防御（如 XSS 转义防范）**。这些都是集成现成插件所学不到的底层与业务经验。”

---

## 2. 前端核心亮点一：乐观更新 (Optimistic UI)

这是大型 C 端项目极度常用的体验提升技巧，能够显著增强交互的即时感与流畅度。

**技术深度探讨：** “如果用户网速很慢，或者由于冷启动导致接口响应稍慢，用户点击了‘发送评论’之后，页面一点反应都没有，你会怎么解决这种交互痛点？”

**核心逻辑表达：**
“其实在设计发送逻辑时就考虑到了网络延迟的问题。常见的做法是：`展现 Loading 动画 -> 等待后端返回成功 -> 重新拉取一次整个评论列表`。这在网络环境复杂时体验受限。
我使用的是**‘乐观更新（Optimistic UI）’策略**：在通过了前置空校验后，我不等网络请求执行完毕，立刻把用户刚刚填写的那条评论内容构建成一个展示模型，直接 `unshift` 到本地的 `comments` 响应式数组最顶端。
对于用户来说，他是**零延迟**地看到了自己的评论出现在页面上。而底层真实的网络请求是在后台静默执行的，以此提供极致流畅的交互体验。”

**追问点：** “那如果底层的那个网络请求真的失败了呢？”
**核心逻辑表达：**
“通过 `try-catch` 捕获到后台请求异常后，我会进行**状态回滚**：将刚才前端数组里第一项（那条乐观更新的数据）`shift` 弹出，并给出友好提示，同时将被清空的输入框内容按备份重新填回，让用户可以随时无缝重试，保证系统的严谨与数据一致性。”

---

## 3. 前端核心亮点二：前端安全之防范 XSS 攻击

对于任何具有“用户任意输入并展示给所有人”的内容区，**XSS（跨站脚本攻击）** 是系统设计的核心考点。

**安全性深度探讨：** “你的留言板允许访客随便输入，如果有人恶意输入了 `<script>alert('...')</script>`，页面会出现弹窗或脚本注入吗？”

**核心逻辑表达：**
“完全不会。在这套系统中我设计了**两道防线**：
1. **渲染层面的天然防御**：在使用 Vue3 渲染评论列表时，我使用的是双花括号插值 `{{ item.content }}` 来输出文本，这确保了 Vue 模板引擎在渲染前会自动对尖括号等非法字符进行 DOM 生态安全的字符实体转义处理，而绝不会使用会导致存储型 XSS 的 `v-html` 指令。
2. **接口存储层的深层拦截**：为了保证即使绕过前端直接调用 API 也无法污染数据库，在 Serverless 端收到 POST 数据的第一时间，我使用正则手动将 `<` 替换为了 `&lt;`，从源头杜绝了危险脚本入库。前后两道防线协同，彻底消除了基本的反射型与存储型 XSS 的风险。”

---

## 4. 其它核心技术细节

除了上述核心亮点，以下技术细节也体现了系统的严谨性：

*   **按钮防抖与锁定**：“考虑到快速连击可能导致脏数据写入，我在点击提交的瞬间挂载了 `isSubmitting` 锁变量，将其绑定到按钮的 `:disabled` 属性以及透明度的动态样式上，并在接口的 `finally` 生命周期里解锁。”
*   **无状态组件分发**：“因为 Valaxy 博客是由很多 Markdown 文件构成的，我把整个逻辑抽离成了高度内敛独立的 `<ArticleComments />` 组件，只需要像之前一样通过 `slug` 传值，就能立刻让任意枯燥的文章获得讨论能力。”
*   **组件双向绑定**：“对于评论昵称和内容框，我采用原生的 `v-model` 进行 Vue 双向状态绑定，相比原生监听事件更加优雅高效。”
